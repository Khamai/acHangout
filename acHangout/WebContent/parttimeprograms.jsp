	...
	<c:if test="${not empty param.page}">
		<%
			int check = Integer.valueOf(request.getParameter("page"));
					session.setAttribute("directpage", "display?topic=parttimeprograms&page=" + check);
		%>
	</c:if>
	<c:if test="${empty param.page}">
		<%
			session.setAttribute("directpage", "display?topic=parttimeprograms&page=1");
		%>
	</c:if>
	...
	<div class="container">
		<div class="row">
			<ul class="breadcrumb">
				...
				<li class="active">Part Time Programs</li>
			</ul>
		</div>
		<br /> <br />
	...
	<div class="container">
		<div class="row mt">
			<div class="col-sm-12 forum-category lpad">
				<h3>Part-Time Programs Posts</h3>
			</div>
		...
		<div class="row">
			<ul class="pagination">
				...
				<c:if test="${currentpage > 1}">
					<li><a href="display?topic=parttimeprograms&page=1">First</a></li>
					<li><a href="display?topic=parttimeprograms&page=${currentpage - 1}">&laquo;</a></li>
				</c:if>
				...
				<c:forEach begin="1" end="${numberofpage}" var="page">
					<c:choose>
						<c:when test="${currentpage eq page }">
							<li class="active"><a
								href="display?topic=parttimeprograms&page=${currentpage}">${page}</a></li>
						</c:when>
						<c:otherwise>
							<li><a href="display?topic=parttimeprograms&page=${page}">${page}</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>

				<c:if test="${currentpage lt numberofpage}">
					<li><a href="display?topic=parttimeprograms&page=${currentpage + 1}">&raquo;</a></li>
					<li><a href="display?topic=parttimeprograms&page=${numberofpage}">Last</a></li>
				</c:if>
